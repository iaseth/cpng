...
void
draw_curtains (struct CpngImage *image, int margin, char *c1, char *c2)
{
	struct CpngColor *color = image->colors[image->current_color_index];

	char filename[100];
	sprintf(filename, "pngs/curtains/curtains_%s_%s.png", c1, c2);
...
curtain_stuff ()
{
	...
	cpng_image_set_title(image, "Curtains");

	draw_curtains(image, 20, "awesome", "burgundy");
